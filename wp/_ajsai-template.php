...
?>
    ...
    <?php 
    // 出力
    ajsai_the_post_thumbnail()?>
    <?php 
    // 取得
    ajsai_get_post_thumbnail()?>
 ...
 *
 -->
<?php
// 現在リクエストされているクエリ情報の取得
// カテゴリ一覧とかで、今表示中のカテゴリを取得するのに便利
get_queried_object();
...
<!--
 * リライトルールを確認
 * パーマリンクをやたらといじったり、ページネーションが複雑な際に使うと、動作しているURLの一覧が観れる
 *
 -->
<?php 
echo 'リライトルール確認';
...
?>
<!--
 * 現在表示しているページで使用されているテーマ名・テンプレート名・テンプレートパスを表示
 * 
 *
 -->
<?php
...
if( !empty( $next_post ) ) {
    ...
    echo '<a href="'.$url .'" class="prev">NEXT<img src="'.get_template_directory_uri().'/assets/images/_common/arrow_left_blue.png" alt=""></a>';
}
?>
<a href="<?php echo get_post_type_archive_link( 'post' ); ?>" class="toArchive"><img src="assets/images/_common/icon_archive.png" alt=""></a>
<?php 
$prev_post = get_previous_post();
if( !empty( $prev_post ) ) {
    $url = get_permalink( $prev_post->ID );
    echo '<a href="'.$url .'" class="next"><img src="'.get_template_directory_uri().'/assets/images/_common/arrow_right_blue.png" alt="">PREV</a>';
}
?>